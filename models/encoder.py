...
class Conv_Pyram_model_HAR_1(nn.Module):
    def __init__(self, input_dims, output_dims, hidden_dims=64, depth=10, mask_mode='binomial', dropout=0.0):
        ...
        self.conv_block3_t = nn.Sequential(
            nn.Conv1d(64, self.output_dims, kernel_size=7, stride=1, bias=False, padding=4),
            nn.BatchNorm1d(self.output_dims),
            nn.MaxPool1d(kernel_size=2, stride=2, padding=1),
            nn.Dropout(self.dropout)
        ).cuda()
...
class Conv_Pyram_model_ISRUC(nn.Module):
    def __init__(self, input_dims, output_dims, dropout=0.2):
        super().__init__()
        self.input_dims = input_dims
        self.output_dims = output_dims
        self.kernel_size = 25
