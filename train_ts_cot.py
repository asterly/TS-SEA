...
from ts_cot_model import TS_SEA
...
def eval_mlp(device,logger,args):
    ...
    logger.info("="*100)
    if args.backbone_type in ['TS_CoT','TS_SEA'] :
    ...
    logger.info(f"The backbone of eval_mlp is {args.backbone_type}")
    logger.info(f"load model path from {args.model_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backbone_type', default='TS_SEA', type=str,
    ...
    parser.add_argument('--run_desc', default="exp_run", type=str, help='The moving average parameter for prototype updating')
    parser.add_argument('--data_perc', default="train", type=str, help='The moving average parameter for prototype updating')
    ...
    logger.info(args)
    if args.backbone_type in ['TS_CoT','TS_SEA'] and not args.eval:
    ...
    if args.eval:
        ...
        #     logger.info(f"Evaluation result: ACC: {eval_res['acc']}   AUROC: {eval_res['auroc']}")
        # else:
        #     logger.info('No loaded pre-trained model.')
